...
export let fetchArticles = (isRefreshing, loading, typeId, isLoadMore, page = 0) => {
  let type = '';
  if (typeId == '0') {//头条
    type = '&cat_id%5B%5D=4815';
  }else if (typeId == '12') {//调查
    type = '&cat_id%5B%5D=4714';
  }else if (typeId == '9') {//时局
    type = '&cat_id%5B%5D=4718';
  }else if (typeId == '8') {//视频
    type = '&cat_id%5B%5D=5140';
  }else if (typeId == '7') {//Hi,南周
    type = '&cat_id%5B%5D=5139';
  }

    let URL = 'http://www.infzm.com/mobile/get_list_by_cat_ids?count=11&platform=ireader&device=Unknown%20iPhone&version=5.1.0&system_version=9.3&hash=f2cdde51e779dd63b44690b9c86b216b&format=json';
    URL += '&';
    URL += 'start=';
    URL += page;
    URL += type;
